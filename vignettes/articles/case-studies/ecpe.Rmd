...
att73_text <- att73 %>% 
  glue_data("a {fmt_prop_pct(probability)}% chance the respondent is proficient on {attribute} rules") %>% 
  combine_words()

att73_morpho <- att73 %>% 
  filter(attribute == "morphosyntactic") %>% 
  glue_data("as low as {fmt_prop_pct(`2.5%`)}% or as high as {fmt_prop_pct(`97.5%`)}%")
```
