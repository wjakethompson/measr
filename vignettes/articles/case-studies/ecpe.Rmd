...
Because the ECPE data consists of 3 dichotomous attributes, there are a total of 2^3^ = 8 possible profiles.
We can extract the structural parameters using `measr_extract()`.
...
The order of the attributes in the profiles corresponds to the order of attributes in the Q-matrix that was used to specify the model.
...
Specifically, respondents are typically proficient on no attribute, only attribute 3 (lexical rules), only attribute 2 and 3 (cohesive and lexical rules), or all attributes.
...
This also echos our analysis of item *p*-values when we were exploring the data, which showed that items measuring morphosyntactic rules were more difficult than items measuring lexical or cohesive rules.
...
```{r}
ecpe_lcdm <- add_respondent_estimates()
```
...
For example, we could determine each respondent's scores by choosing the overall profile that is most consistent with their observed responses.
...
On the other hand, rather than basing results on the overall most likely profile, we could score each attribute individually.
...
In this instance, because the probabilities are reported, we would want to report the reliability or precision of that probability estimate.
This is type of result is known as the *expected a posteriori* (EAP) estimate because it is the expected value of the classification.
...
For example, say that our threshold for proficiency was a probability of proficiency greater than .5.
A respondent could have a probability anywhere from .5--1.0 and get the same classification, so a less reliable EAP would still result in the same MAP classification.
