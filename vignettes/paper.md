...
With Stan as a backend, measr can estimate a wide variety of DCMs, such as the loglinear cognitive diagnostic model [LCDM; @lcdm]; the deterministic inputs, noisy "and" gate [DINA; @dina] model; and the deterministic inputs, noisy "or" gate [DINO; @dino] model.
...
However, models estimated with measr also include the fitted Stan object so users can access it if they are familiar with Stan and prefer to work with those object.
