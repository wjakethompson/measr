  ...
  out <- capture.output(
    suppressMessages(
      cmds_mdm_lcdm <- dcm_estimate(
        lcdm_spec,
        data = dcmdata::mdm_data, identifier = "respondent", missing = NA,
  ...
  out <- capture.output(
    suppressMessages(
      rstn_mdm_dina <- dcm_estimate(
        ...
        iter = 1500, warmup = 1000, chains = 2, cores = 2, refresh = 0
      )
    )
  )
}
...
test_that("model comparisons work", {
  ...
  expect_equal(colnames(lcdm_save),
               c("elpd_diff", "se_diff", "elpd_loo", "se_elpd_loo",
                 "p_loo", "se_p_loo", "looic", "se_looic"))
  ...
  expect_equal(colnames(dina_save),
  ...
  expect_equal(colnames(all_save),
               c("elpd_diff", "se_diff", "elpd_loo", "se_elpd_loo",
                 "p_loo", "se_p_loo", "looic", "se_looic"))

  err <- rlang::catch_cnd(loo_compare(lcdm_compare, dina_compare,
...
test_that("ppmc works", {
  ...
  # test 1 -----
  test_ppmc <- fit_ppmc(cmds_mdm_lcdm, ndraws = 500, return_draws = 100,
                        model_fit = "raw_score",
  ...
  expect_equal(colnames(test_ppmc$ppmc_raw_score),
  ...
  expect_equal(colnames(test_ppmc$ppmc_conditional_prob),
               c("item", "class", "obs_cond_pval", "ppmc_mean", "2.5%", "97.5%",
                 "samples", "ppp"))
  expect_equal(as.character(test_ppmc$ppmc_conditional_prob$item),
               rep(paste0("mdm", 1:4), each = 2))
  expect_equal(as.character(test_ppmc$ppmc_conditional_prob$class),
               rep(c("[0]", "[1]"), 4))
  expect_equal(vapply(test_ppmc$ppmc_conditional_prob$samples,
                      length, integer(1)),
               rep(100, 8))

  # test 2 -----
  test_ppmc <- fit_ppmc(rstn_mdm_dina, ndraws = 200, return_draws = 180,
                        probs = c(0.055, 0.945),
  ...
  expect_equal(colnames(test_ppmc$ppmc_odds_ratio),
               c("item_1", "item_2", "obs_or", "ppmc_mean", "5.5%", "94.5%",
                 "samples", "ppp"))
  expect_equal(as.character(test_ppmc$ppmc_odds_ratio$item_1),
               c(rep("mdm1", 3), rep("mdm2", 2), "mdm3"))
  expect_equal(as.character(test_ppmc$ppmc_odds_ratio$item_2),
               c("mdm2", "mdm3", "mdm4", "mdm3", "mdm4", "mdm4"))
  expect_equal(vapply(test_ppmc$ppmc_odds_ratio$samples,
                      length, integer(1)),
               rep(180, 6))

  expect_s3_class(test_ppmc$ppmc_pvalue, "tbl_df")
  expect_equal(nrow(test_ppmc$ppmc_pvalue), 4)
  expect_equal(colnames(test_ppmc$ppmc_pvalue),
               c("item", "obs_pvalue", "ppmc_mean", "5.5%", "94.5%",
                 "samples", "ppp"))
  expect_equal(as.character(test_ppmc$ppmc_pvalue$item),
               paste0("mdm", 1:4))
  expect_equal(vapply(test_ppmc$ppmc_pvalue$samples,
                      length, double(1)),
               rep(180, 4))
...
test_that("model fit can be added", {
  ...
  expect_equal(names(test_model@fit$ppmc_odds_ratio),
               c("item_1", "item_2", "obs_or", "ppmc_mean", "2.5%", "97.5%",
                 "samples", "ppp"))
  expect_identical(test_model@fit[-which(names(test_model@fit) == "m2")],
  ...
  expect_equal(names(test_model@fit$ppmc_raw_score),
               c("obs_chisq", "ppmc_mean", "5.5%", "94.5%", "ppp"))
  expect_equal(names(test_model@fit$ppmc_odds_ratio),
               c("item_1", "item_2", "obs_or", "ppmc_mean", "2.5%", "97.5%",
                 "samples", "ppp"))
  expect_equal(names(test_model@fit$ppmc_conditional_prob),
  ...
  expect_identical(test_ppmc$ppmc_conditional_prob,
                   test_model@fit$ppmc_conditional_prob)
  expect_equal(names(test_ppmc$ppmc_pvalue),
               c("item", "obs_pvalue", "ppmc_mean", "2.5%", "97.5%", "ppp"))
  ...
  expect_equal(names(test_model@fit$ppmc_conditional_prob),
               c("item", "class", "obs_cond_pval", "ppmc_mean", "10%", "90%",
                 "samples", "ppp"))
  expect_equal(names(test_model@fit$ppmc_pvalue),
  ...
  expect_equal(or_check,
               test_model@fit$ppmc_odds_ratio)
...
test_that("respondent probabilities are correct", {
  ...
  mdm_preds <- score(cmds_mdm_lcdm, newdata = dcmdata::mdm_data,
                     identifier = "respondent", summary = TRUE)
